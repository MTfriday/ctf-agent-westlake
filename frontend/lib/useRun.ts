...
// ── Aemeath：自建 agent 直接接入 LLM 平台（不用 claude/codex CLI worker）──
// 配置面板（WorkerSettings）读写这份契约。后端 /api/settings/workers。
// 支持多个 LLM 端点（provider），当前启用一个（active）。
export interface AemeathLlmProvider {
  id: string;
  label: string;            // 显示名（阿里百炼 / DeepSeek …）
  base_url: string;         // OpenAI 兼容端点
  api_key_present: boolean; // 只读：该 provider 的 key 是否在环境里
  model_planner: string;    // 协调器规划模型（qwen3.7-max）
  model_router: string;     // 路由模型（qwen3.6-flash）
  enabled: boolean;
}

export interface AemeathLlmConfig {
  active: string;                       // 当前启用的 provider id
  providers: AemeathLlmProvider[];
}

export interface AemeathAgentConfig {
  engine_backend: string;  // swarm | hybrid | orchestrated
  worker_count: number;    // 并发 worker 数
  race_scout: boolean;     // 竞速侦察（race scout）开关
  race_timeout: number;    // 竞速侦察超时（秒）
  wall_clock_budget: number;
  cost_budget_usd: number;
}

export interface AemeathConfig {
  llm: AemeathLlmConfig;
  agent: AemeathAgentConfig;
}

export async function getAemeathConfig(): Promise<AemeathConfig | null> {
  try {
    const r = await apiFetch(`/api/settings/workers`);
    if (!r.ok) return null;
    const j = await r.json();
    return (j.config ?? null) as AemeathConfig | null;
  } catch {
    return null;
  }
}

export async function putAemeathConfig(
  patch: Partial<{ llm: Partial<AemeathLlmConfig>; agent: Partial<AemeathAgentConfig> }>
): Promise<AemeathConfig | null> {
  try {
    const r = await apiFetch(`/api/settings/workers`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
...
/** USD→CNY 汇率（在线获取，后端兜底 7.2）。用于成本预算双币种联动。 */
export async function getRate(): Promise<{ usd_cny: number; source: string } | null> {
  try {
    const r = await apiFetch(`/api/settings/rate`);
    if (!r.ok) return null;
    const j = await r.json();
    return { usd_cny: Number(j.usd_cny ?? 7.2), source: String(j.source ?? "") };
  } catch {
    return null;
  }
}
